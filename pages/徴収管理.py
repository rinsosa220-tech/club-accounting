...
with col_left:
    ...
    if IS_ADMIN:
        with st.expander("🛠️ 管理者専用ツール"):
            if st.button("🚨 部員名簿を最新の学年順で再構築する (1回だけ押してください)"):
                from utils.sheets import load_members, save_members
                MEMBER_ORDER = [
                ...
                mgr_start = MEMBER_ORDER.index("安藤　宇乃")
                roles = ["Player"] * mgr_start + ["Manager"] * (len(MEMBER_ORDER) - mgr_start)
                ...
                st.success("✅ 部員名簿を再構築しました！ページをリロードしてください。")
                st.rerun()
